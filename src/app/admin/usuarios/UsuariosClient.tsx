...
function PointsBadge({ points }: { points: number | null }) {
  if (points === null) return <span className="text-[#e0d0f8] text-xs">—</span>;
  const color = points === 12 ? "text-yellow-400" : points >= 5 ? "text-green-400" : points > 0 ? "text-blue-300" : "text-[#c4a7f0]";
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        ...
        <select
          ...
          className="flex-1 bg-[#2d1a5e] border border-white/20 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-[#6b3db8]"
        >
      ...
      {selectedUser && (
        <>
          {/* Info + cambio de rol */}
          <div className="bg-[#2d1a5e] border border-white/10 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-white font-bold text-sm">{selectedUser.alias}</p>
              <p className="text-[#c4a7f0] text-xs truncate">{selectedUser.email}</p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                ...
                className="text-[10px] font-bold px-2 py-1 rounded-lg border border-white/20 text-[#c4a7f0] hover:text-white hover:border-[#6b3db8] transition-colors disabled:opacity-40"
              >
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-[#2d1a5e] border border-white/10 rounded-xl p-3 text-center">
              ...
              <div className="text-[#c4a7f0] text-[10px]">puntos</div>
            </div>
            <div className="bg-[#2d1a5e] border border-white/10 rounded-xl p-3 text-center">
              <div className="text-xl font-black text-yellow-400">{exactos}</div>
              <div className="text-[#c4a7f0] text-[10px]">exactos</div>
            </div>
            <div className="bg-[#2d1a5e] border border-white/10 rounded-xl p-3 text-center">
              <div className="text-xl font-black text-[#e0d0f8]">{filled}</div>
              <div className="text-[#c4a7f0] text-[10px]">cargados</div>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="text-[#c4a7f0] text-[10px] uppercase tracking-wider">
                  ...
                  return (
                    <tr key={m.id} className="border-t border-white/10">
                      <td className="py-2 pr-4">
                        ...
                        <div className="flex items-center gap-1 text-xs text-[#e0d0f8]">
                          <Flag code={m.away_flag} /> {m.away_team ?? "TBD"}
                        </div>
                      </td>
                      <td className="text-center text-white font-bold text-sm">
                        {m.is_played ? `${m.home_score_real}-${m.away_score_real}` : <span className="text-[#e0d0f8]">—</span>}
                      </td>
                      <td className="text-center text-[#e0d0f8] font-bold text-sm">
                        {pred ? `${pred.home_score}-${pred.away_score}` : <span className="text-[#e0d0f8]">—</span>}
                      </td>
