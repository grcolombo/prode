  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-[#c4a7f0] text-xs mt-1">
      ...
      <div className="flex gap-2">
        <input
          ...
          disabled={loading}
          className="flex-1 bg-[#2d1a5e] border border-white/20 rounded-xl px-4 py-2 text-white text-sm focus:outline-none focus:border-[#6b3db8] disabled:opacity-50"
        />
        <button
          onClick={handleAdd}
      ...
      <div className="flex flex-col gap-1">
        <p className="text-[10px] text-[#c4a7f0] font-bold uppercase tracking-wider mb-1">
          {emails.length} emails registrados
        </p>
        {emails.length === 0 ? (
          <p className="text-[#e0d0f8] text-sm text-center py-8">Sin emails cargados</p>
        ) : (
          emails.map(email => (
            <div
              key={email}
              className="flex items-center justify-between bg-[#2d1a5e] border border-white/10 rounded-xl px-4 py-2.5"
            >
              <span className="text-sm text-[#d4c0f0]">{email}</span>
              <button
                onClick={() => handleDelete(email)}
                disabled={deletingEmail === email}
                className="text-[#c4a7f0] hover:text-red-400 text-xs font-bold transition-colors disabled:opacity-40 ml-4"
              >
